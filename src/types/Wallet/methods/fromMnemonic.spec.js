...
describe("Wallet - fromMnemonic", function suite() {
  ...
  it("should indicate missing data", () => {
    ...
    const exceptedException1 =
      "Expected a valid mnemonic (typeof String or Mnemonic)";
  ...
  it("should set wallet from mnemonic", () => {
    ...
    expect(self1.HDPrivateKey.toString()).to.equal(
      knifeFixture.HDRootPrivateKeyMainnet
    );
    expect(new Dashcore.HDPrivateKey(self1.HDPrivateKey)).to.equal(
      self1.HDPrivateKey
    );
    expect(self1.keyChain.type).to.equal("HDPrivateKey");
    expect(self1.keyChain.network.name).to.equal("livenet");
    expect(self1.keyChain.HDPrivateKey.toString()).to.equal(
      knifeFixture.HDRootPrivateKeyMainnet
    );
    ...
    expect(self2.HDPrivateKey.toString()).to.equal(
      knifeFixture.HDRootPrivateKeyTestnet
    );
    expect(new Dashcore.HDPrivateKey(self2.HDPrivateKey)).to.equal(
      self2.HDPrivateKey
    );
    expect(self2.keyChain.type).to.equal("HDPrivateKey");
    expect(self2.keyChain.HDPrivateKey.toString()).to.equal(
      knifeFixture.HDRootPrivateKeyTestnet
    );
  ...
  it("should reject invalid mnemonic", () => {
    const invalidInputs = [
      {
        mnemonic:
          "knife easily prosper input concert merge prepare autumn pen blood glance chair",
      },
      { mnemonic: false },
      { mnemonic: true },
      { mnemonic: 0 },
    ...
    return invalidInputs.forEach((invalidInput) => {
      const self = {};
      expect(() => fromMnemonic.call(self, invalidInput)).to.throw(
        "Expected a valid mnemonic (typeof String or Mnemonic)"
      );
    });
  });
...
describe("Wallet - fromMnemonic - with passphrase", function suite() {
  ...
  it("should correctly works with passphrase", () => {
    ...
    expect(self1.mnemonic).to.equal(knifeFixture.mnemonic);
    expect(self1.HDPrivateKey.toString()).to.equal(
      knifeFixture.HDRootEncryptedPrivateKeyMainnet
    );
    expect(new Dashcore.HDPrivateKey(self1.HDPrivateKey)).to.equal(
      self1.HDPrivateKey
    );
    ...
    expect(new Dashcore.Address(pubKey1).toString()).to.equal(
      "Xq3zjky18WjwAHpLgGLasvX5g8TeLRKaxt"
    );
    ...
    expect(self2.mnemonic).to.equal(knifeFixture.mnemonic);
    expect(self2.HDPrivateKey.toString()).to.equal(
      knifeFixture.HDRootEncryptedPrivateKeyTestnet
    );
    expect(new Dashcore.HDPrivateKey(self2.HDPrivateKey)).to.equal(
      self2.HDPrivateKey
    );
    ...
    expect(new Dashcore.Address(pubKey2, "testnet").toString()).to.equal(
      "yWYCH9XDRnpdNxh67jQJFkovToBVwWr8Ck"
    );
  });
});
