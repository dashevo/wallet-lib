...
describe("Wallet - sweepWallet", function suite() {
  ...
  it("should warn on empty balance", async () => {
    ...
    const exceptedException =
      "Cannot sweep an empty private key (current balance: 0)";
    await expectThrowsAsync(
      async () => await emptyWallet.sweepWallet(),
      exceptedException
    );
    await emptyWallet.disconnect();
  });
  it("should warn on sweep from mnemonic", async () => {
    const exceptedException =
      "Can only sweep wallet initialized from privateKey";
    const mockWallet = {
      walletType: "HDWALLET",
      getAccount: () => ({
    ...
    expectThrowsAsync(
      async () => await sweepWallet.call(mockWallet),
      exceptedException
    );
  });
});
