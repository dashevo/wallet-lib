...
describe("Wallet - HDPublicKey", function suite() {
  ...
  it("should detect wrong parameters", () => {
    ...
    const exceptedException1 =
      "Expected a valid HDPublicKey (typeof HDPublicKey or String)";
    expect(() => fromHDPublicKey.call(mockOpts1)).to.throw(exceptedException1);
    expect(() =>
      fromHDPublicKey.call(mockOpts1, gatherTestnet.external.hdprivkey)
    ).to.throw(exceptedException1);
    expect(() =>
      fromHDPublicKey.call(mockOpts1, gatherTestnet.mnemonic)
    ).to.throw(exceptedException1);
    expect(() =>
      fromHDPublicKey.call(
        mockOpts1,
  ...
  it("should work from a valid HDPubKey", () => {
    ...
    expect(mockOpts1.HDPublicKey.toString()).to.equal(
      gatherTestnet.external.hdpubkey
    );
    expect(new Dashcore.HDPublicKey(mockOpts1.HDPublicKey)).to.equal(
      mockOpts1.HDPublicKey
    );
    expect(mockOpts1.keyChain.type).to.equal("HDPublicKey");
    expect(mockOpts1.keyChain.HDPublicKey).to.deep.equal(
      Dashcore.HDPublicKey(gatherTestnet.external.hdpubkey)
    );
    expect(mockOpts1.keyChain.keys).to.deep.equal({});
  });
  it("should work from a HDPubKey", () => {
    const wallet1 = new Wallet({
      HDPublicKey: gatherTestnet.external.hdpubkey,
      offlineMode: true,
    });
    ...
    expect(wallet1.network).to.be.deep.equal(
      Dashcore.Networks.testnet.toString()
    );
