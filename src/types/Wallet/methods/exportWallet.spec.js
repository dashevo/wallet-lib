...
describe("Wallet - export Wallet", function suite() {
  ...
  it("should indicate on missing data", () => {
    ...
    expect(() => exportWallet.call(mockOpts3, "mnemonic")).to.throw(
      exceptedException3
    );
  });
  it("should export a privateKey", () => {
    ...
    expect(exportWallet.call(mockOpts3)).to.equal(
      knifeMnemonic.HDRootPrivateKeyMainnet
    );
  });
});
describe("Wallet - exportWallet - integration", function suite() {
  ...
  describe("fromMnemonic", () => {
    ...
    it("should work as expected", () => {
      ...
      expect(wallet.exportWallet("HDPrivateKey")).to.equal(
        knifeMnemonic.HDRootPrivateKeyTestnet
      );
  ...
  describe("fromHDPublicKey", () => {
    ...
    it("should work as expected", () => {
      ...
      const exceptedException2 = "Tried to export to invalid output : seed";
      const exceptedException3 =
        "Tried to export to invalid output : HDPrivateKey";

      expect(wallet.exportWallet()).to.equal(
        knifeMnemonic.HDRootPublicKeyMainnet
      );
      expect(() => wallet.exportWallet("mnemonic")).to.throw(exceptedException);
      expect(() => wallet.exportWallet("seed")).to.throw(exceptedException2);
      expect(() => wallet.exportWallet("HDPrivateKey")).to.throw(
        exceptedException3
      );
      expect(wallet.exportWallet("HDPublicKey")).to.equal(
        knifeMnemonic.HDRootPublicKeyMainnet
      );
    });
    after(() => {
