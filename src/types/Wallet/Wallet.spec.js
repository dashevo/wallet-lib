...
describe("Wallet - class", function suite() {
  ...
  it("should create a wallet without parameters", () => {
    ...
    expect(Dashcore.Mnemonic(wallet1.mnemonic).toString()).to.be.equal(
      wallet1.mnemonic
    );
    ...
    expect(Dashcore.Mnemonic(wallet2.mnemonic).toString()).to.be.equal(
      wallet2.mnemonic
    );
    expect(wallet2.mnemonic).to.be.not.equal(wallet1.mnemonic);
    expect(wallet2.network).to.be.deep.equal(
      Dashcore.Networks.testnet.toString()
    );
  ...
  it("should create a wallet with mnemonic", () => {
    ...
    expect(wallet2.walletType).to.be.equal(WALLET_TYPES.HDWALLET);
    expect(wallet2.network).to.be.deep.equal(
      Dashcore.Networks.mainnet.toString()
    );
    expect(Dashcore.Mnemonic(wallet2.mnemonic).toString()).to.be.equal(
      wallet2.mnemonic
    );
  ...
  it("should create a wallet with HDPublicKey", () => {
    const wallet1 = new Wallet({
      HDPublicKey: gatherSailMnemonic.testnet.external.hdpubkey,
      network: "testnet",
      ...mocks,
    });
    ...
    expect(wallet1.allowSensitiveOperations).to.be.deep.equal(false);
    expect(wallet1.injectDefaultPlugins).to.be.deep.equal(true);
    expect(wallet1.walletId).to.be.equal(
      gatherSailMnemonic.testnet.external.walletId
    );
    wallet1.storage.on("CONFIGURED", () => {
      wallet1.disconnect();
    });
...
describe("Wallet - Get/Create Account", function suite() {
  ...
  it("should encrypt wallet with a passphrase", () => {
    ...
    const expectedHDPriv =
      "tprv8ZgxMBicQKsPcuZMDBeTL2qaBF7gyUPt2wbqbJG2yp8s7yzRE1cRcjRnG3Xmdv3sELwtLGz186VX3EeHQ5we1xr1qH95QN6FRopP6FZqBUJ";
  ...
  it("should be able to create an account at a specific index", (done) => {
    ...
    walletTestnet.createAccount().then(async (account) => {
      ...
      const accountSpecificIndex = await walletTestnet.createAccount({
        index: 42,
      });
