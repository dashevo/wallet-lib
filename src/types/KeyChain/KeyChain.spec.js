...
describe("Keychain", function suite() {
  ...
  it("should derive from hardened feature path", () => {
    const hardenedPk = keychain.getHardenedBIP44Path();
    const derivedPk = hardenedPk
      .deriveChild(0, true)
      .deriveChild(0)
      .deriveChild(0);
    const address = new Dashcore.Address(
      derivedPk.publicKey.toAddress()
    ).toString();
    expect(address).to.equal("yNfUebksUc5HoSfg8gv98ruC3jUNJUM8pT");
  });
  it("should generate key for child", () => {
    ...
    expect(keyForChild.toString()).to.equal(
      "tprv8d4podc2Tg459CH2bwLHXj3vdJFBT2rdsk5Nr1djH7hzHdt5LRdvN6QyFwMiDy7ffRdik7fEVRKKgsHB4F18sh8xF6jFXpKq4sUgGBoSbKw"
    );
  });
...
describe("Keychain - single privateKey", function suite() {
  ...
  it("should correctly errors out when not a HDPublicKey (privateKey)", () => {
    ...
    const expectedException1 =
      "Wallet is not loaded from a mnemonic or a HDPubKey, impossible to derivate keys";
    const expectedException2 =
      "Wallet is not loaded from a mnemonic or a HDPubKey, impossible to derivate child";
