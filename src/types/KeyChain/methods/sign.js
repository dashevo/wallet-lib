const { crypto, Transaction, Message } = require("@dashevo/dashcore-lib");
...
function sign(object, privateKeys, sigType = crypto.Signature.SIGHASH_ALL) {
  const handledTypes = [
    Transaction.name,
    Transaction.Payload.SubTxRegisterPayload,
    Message.name,
  ];
  if (!privateKeys)
    throw new Error("Require one or multiple privateKeys to sign");
  if (!object) throw new Error("Nothing to sign");
  if (!handledTypes.includes(object.constructor.name)) {
    throw new Error(
      `Keychain sign : Unhandled object of type ${object.constructor.name}`
    );
  }
