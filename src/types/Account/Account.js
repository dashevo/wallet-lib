...
function getNextUnusedAccountIndexForWallet(wallet) {
  ...
  throw new Error(
    "An account is attached to a wallet that has not been provided to the account constructor."
  );
}
...
class Account extends EventEmitter {
  constructor(wallet, opts = defaultOptions) {
    super();
    if (!wallet || wallet.constructor.name !== Wallet.name)
    ...
    this.index = _.has(opts, "index")
      ? opts.index
      : getNextUnusedAccountIndexForWallet(wallet);
    ...
    this.on("error", (error, errorContext) =>
      wallet.emit("error", error, {
        ...errorContext,
    ...
    this.storage.on(EVENTS.REHYDRATE_STATE_FAILED, (ev) =>
      this.emit(ev.type, ev)
    );
    this.storage.on(EVENTS.REHYDRATE_STATE_SUCCESS, (ev) =>
      this.emit(ev.type, ev)
    );
    this.storage.on(EVENTS.FETCHED_CONFIRMED_TRANSACTION, (ev) =>
      this.emit(ev.type, ev)
    );
    this.storage.on(EVENTS.UNCONFIRMED_BALANCE_CHANGED, (ev) =>
      this.emit(ev.type, ev)
    );
    this.storage.on(EVENTS.CONFIRMED_BALANCE_CHANGED, (ev) =>
      this.emit(ev.type, ev)
    );
    ...
    if (this.debug) {
      this.emit = (...args) => {
        ...
        logger.debug(
          `${this.walletId}:${this.index} - Emitted event ${type} - ${payload} `
        );
        super.emit(...args);
      };
    }
    if (
      [WALLET_TYPES.HDWALLET, WALLET_TYPES.HDPUBLIC].includes(this.walletType)
    ) {
    ...
    if (this.walletType === WALLET_TYPES.SINGLE_ADDRESS) {
      this.storage.createSingleAddress(this.walletId, this.network, this.label);
    }
    ...
    return Promise.race([
      ...
      new Promise((resolve, reject) => {
        setTimeout(() => {
          reject(
            new Error(
              `InstantLock waiting period for transaction ${transactionHash} timed out`
            )
          );
        }, timeout);
      }),
