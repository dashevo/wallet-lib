...
const getTypePathFromWalletType = (
  walletType,
...
function getAddress(index = 0, _type = "external") {
  const { type, path } = getTypePathFromWalletType(
    this.walletType,
    _type,
    index,
    this.BIP44PATH
  ...
  return matchingTypeAddresses[path]
    ? matchingTypeAddresses[path]
    : this.generateAddress(path);
}
