...
describe("Account - injectPlugin", function suite() {
  ...
  it("should prevent sensible access", async function () {
    ...
    await expectThrowsAsync(
      async () => await injectPlugin.call(mockedSelf, WorkingWorker),
      expectedException1
    );
  });
  ...
  it("should handle faulty worker", async function () {
    ...
    await expectThrowsAsync(
      async () => await injectPlugin.call(mockedSelf, FaultyWorker, true),
      expectedException1
    );
    ...
    expect(mockedSelf.plugins.workers["faultyworker"].isWorkerRunning).to.equal(
      false
    );
    expect(mockedSelf.plugins.workers["faultyworker"].state.started).to.equal(
      false
    );
  });
});
