...
describe("Account - broadcastTransaction", function suite() {
  ...
  beforeEach(() => {
    ...
    keysToSign = [
      new PrivateKey(
        "26d6b24119d1a71de6372ea2d3dc22a014d37e4828b43db6936cb41ea461cce8"
      ),
    ];
  ...
  it("should throw error on missing transport", async () => {
    const expectedException1 =
      "A transport layer is needed to perform a broadcast";
    const self = {
      transport: null,
    };
    expectThrowsAsync(
      async () =>
        await broadcastTransaction.call(self, validRawTxs.tx2to2Testnet),
      expectedException1
    );
  ...
  it("should throw error on invalid rawtx (string)", async () => {
    const expectedException1 =
      "A valid transaction object or it's hex representation is required";
    const self = {
      transport: {},
    };
    ...
    expectThrowsAsync(
      async () =>
        await broadcastTransaction.call(self, invalidRawTxs.notRelatedString),
      expectedException1
    );
  });
  it("should throw error on invalid rawtx (hex)", async () => {
    ...
    expectThrowsAsync(
      async () =>
        await broadcastTransaction.call(self, invalidRawTxs.truncatedRawTx),
      expectedException1
    );
  });
  ...
  it("should update affected tx", () => {
    ...
    const self = {
      ...
      storage: {
        ...
        searchAddressesWithTx: (affectedTxId) => {
          searchCalled = +1;
          return { results: [] };
        },
      },
