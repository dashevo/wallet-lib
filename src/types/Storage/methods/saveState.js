...
const saveState = async function saveState() {
  if (this.autosave && this.adapter && this.adapter.setItem) {
    const self = this;
    try {
      await this.adapter.setItem("transactions", {
      ...
      await this.adapter.setItem("instantLocks", {
        ...self.store.instantLocks,
      });
      this.lastSave = +new Date();
      this.emit(SAVE_STATE_SUCCESS, {
        type: SAVE_STATE_SUCCESS,
        payload: this.lastSave,
      });
      return true;
    } catch (err) {
