...
describe("Storage - updateAddress", function suite() {
  ...
  it("should throw errors on failed update", () => {
    const self = {};
    expect(() => updateAddress.call(self)).to.throw(
      "Expected walletId to update an address"
    );

    expect(() => updateAddress.call(self, {}, "123ae")).to.throw(
      "Address should have property path of type string"
    );
  });
  it("should update an address", () => {
    ...
    const expectedMappedAddress = {
      yLhsYLXW5sFHLDPLj2EHgrmQRhP712ANda: {
        walletId: "a3771aaf93",
        type: "external",
        path: "m/44'/1'/0'/0/0",
      },
    ...
    expect(
      self.store.wallets.a3771aaf93.addresses.external["m/44'/1'/0'/0/0"]
    ).to.deep.equal(expectedUpdatedAddress);
  });
});
