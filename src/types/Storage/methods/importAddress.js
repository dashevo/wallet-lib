...
const importAddress = function importAddress(addressObj, walletId) {
  ...
  if (is.undef(walletId))
    throw new Error("Expected walletId to import an address");
  ...
  const index =
    addressObj.index !== undefined
      ? addressObj.index
      : parseInt(path.split("/")[5], 10);
  ...
  if (addressesStore[type][path]) {
    if (
      addressesStore[type][path].fetchedLast < modifiedAddressObject.fetchedLast
    ) {
