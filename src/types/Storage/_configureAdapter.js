  ...
  if (argAdapterContructorName === "Function") {
    ...
    if (adapter.config) {
      try {
        ...
        throw new Error(
          `Tried to config the adapter. Failed with reason ${e.message}`
        );
      }
    } else if (adapter.createInstance)
      await adapter.createInstance({ name: "dashevo-wallet-lib" });
  } else if (argAdapterContructorName === "Object") {
    if (argAdapter.createInstance)
      throw new Error("Adapter instance not created");
    adapter = argAdapter;
  } else {
