...
async function onStreamData(self, data, addresses, network) {
  /* First check if any instant locks appeared */
  const instantLocksReceived = this.constructor.getInstantSendLocksFromResponse(data);
  ...
  const transactionsFromResponse = this.constructor
    .getTransactionListFromStreamResponse(data);
  ...
  const merkleBlockFromResponse = this.constructor
    .getMerkleBlockFromStreamResponse(data);
