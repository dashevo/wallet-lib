...
module.exports = async function syncUpToTheGapLimit({
  ...
  logger.debug(
    `syncing up to the gap limit: - from block: ${
      fromBlockHash || fromBlockHeight
    } Count: ${count}`
  );
  ...
  const stream = await this.transport.subscribeToTransactionsWithProofs(
    addresses,
    options
  );
  ...
  return new Promise((resolve, reject) => {
    stream
      .on("data", async (response) => {
        ...
        const instantLocksReceived =
          this.constructor.getInstantSendLocksFromResponse(response);
        instantLocksReceived.forEach((isLock) => {
          this.importInstantLock(isLock);
        });

        /* Incoming transactions handling */
        const transactionsFromResponse =
          this.constructor.getTransactionListFromStreamResponse(response);
        const walletTransactions = this.constructor.filterWalletTransactions(
        ...
        if (walletTransactions.transactions.length) {
          const addressesGeneratedCount = await self.importTransactions(
          ...
          if (reachedGapLimit) {
            logger.silly(
              "TransactionSyncStreamWorker - end stream - new addresses generated"
            );
        ...
        const merkleBlockFromResponse =
          this.constructor.getMerkleBlockFromStreamResponse(response);

        if (merkleBlockFromResponse) {
          // Reverse hashes, as they're little endian in the header
          const transactionsInHeader = merkleBlockFromResponse.hashes.map(
            (hashHex) => Buffer.from(hashHex, "hex").reverse().toString("hex")
          );
          const transactionsInWallet = Object.keys(
            self.storage.getStore().transactions
          );
          const isTruePositive = isAnyIntersection(
            transactionsInHeader,
            transactionsInWallet
          );
          if (isTruePositive) {
