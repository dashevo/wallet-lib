...
module.exports = async function startHistoricalSync(network) {
  ...
  try {
    ...
    logger.debug(
      `TransactionSyncStreamWorker - HistoricalSync - Started from ${
        options.fromBlockHash || options.fromBlockHeight
      }, count: ${count}`
    );
    ...
    if (GRPC_RETRY_ERRORS.includes(e.code)) {
      if (this.stream === null && e.code === GrpcErrorCodes.CANCELLED) {
        ...
        logger.debug(
          "TransactionSyncStreamWorker - HistoricalSync - The Worker is stopped"
        );
        return;
      }

      logger.debug(
        "TransactionSyncStreamWorker - HistoricalSync - Restarting the stream"
      );
  ...
  logger.debug(
    `TransactionSyncStreamWorker - HistoricalSync - Synchronized ${count} in ${
      +new Date() - start
    }ms`
  );
};
