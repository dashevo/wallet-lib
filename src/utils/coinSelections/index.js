...
module.exports = function coinSelection(
  utxosList,
  outputsList,
  deductFee = false,
  feeCategory = "normal",
  strategy = STRATEGIES.simpleDescendingAccumulator
) {
  if (!utxosList) {
    throw new Error("A utxosList is required");
  }
  if (utxosList.constructor.name !== Array.name) {
    throw new Error("UtxosList is expected to be an array of utxos");
  }
  if (utxosList.length < 1) {
    throw new Error("utxosList must contain at least 1 utxo");
  }
  if (!outputsList) {
    throw new Error(
  ...
  if (outputsList.constructor.name !== Array.name) {
    throw new Error("outputsList must be an array of outputs");
  }
  if (outputsList.length < 1) {
    throw new Error("outputsList must contains at least 1 output");
  }
