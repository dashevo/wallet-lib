...
const is = {
  ...
  mnemonic: mnemonic => is.string(mnemonic) || mnemonic.constructor.name === Mnemonic.name,
  network: network => !is.null(network) && (is.string(network) || network.constructor.name === Networks.livenet.name),
  privateKey: pKey => pKey.constructor.name === PrivateKey.name || (is.string(pKey) && PrivateKey.isValid(pKey)),
  HDPrivateKey: hdKey => hdKey.constructor.name === HDPrivateKey.name || (is.string(hdKey) && HDPrivateKey.isValidSerialized(hdKey)),
