...
describe("Utils", function suite() {
  ...
  it("should handle dash2Duff", () => {
    ...
    expect(dashToDuffs(-2000)).to.equal(-200000000000);
    expect(() => dashToDuffs("deuxmille")).to.throw(
      "Can only convert a number"
    );
  });
  it("should handle duff2Dash", () => {
    expect(duffsToDash(200000000000)).to.equal(2000);
    expect(duffsToDash(-200000000000)).to.equal(-2000);
    expect(() => duffsToDash("deuxmille")).to.throw(
      "Can only convert a number"
    );
  });
  ...
  it("should convert mnemonic to seed", () => {
    ...
    expect(seed.toString()).to.equal(
      knifeEasilyFixture.HDRootPrivateKeyTestnet
    );
  });
  it("should throw error when mnemonic is not provided in mnemonicToHDPrivateKey", () => {
    const network = Networks.testnet;
    expect(() => mnemonicToHDPrivateKey("", network)).to.throw(
      "Expect mnemonic to be provide"
    );
  });
  ...
  it("should is.mnemonic work", () => {
    ...
    const mnemonic2 =
      "crack spice venue ticket vacant steak next stomach amateur review okay curtain";
  ...
  it("should is.transactionObj work", () => {
    const validTransaction =
      figureBridgeFixture.transactions[
