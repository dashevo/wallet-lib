...
const utxosListAsUnspentOutput = utxosList.map((utxo) =>
  Transaction.UnspentOutput(utxo)
);
const outputs = {
  ONE_DASH: {
    satoshis: 100000000,
...
describe("Utils - coinSelection", function suite() {
  ...
  it("should require a utxosList as an array", () => {
    expect(() => coinSelection({})).to.throw(
      "UtxosList is expected to be an array of utxos"
    );
  });
  ...
  it("should require a outputsList", () => {
    expect(() => coinSelection(utxosList)).to.throw(
      "An outputsList is required in order to perform a selection"
    );
  });
  // return;
  it("should require a outputsList as an array", () => {
    expect(() => coinSelection(utxosListAsUnspentOutput, {})).to.throw(
      "outputsList must be an array of outputs"
    );
  });
  it("should require a outputsList with at least one output", () => {
    expect(() => coinSelection(utxosList, [])).to.throw(
      "outputsList must contains at least 1 output"
    );
  });
  it("should require a outputsList with valid outputs", () => {
    expect(() =>
      coinSelection(utxosListAsUnspentOutput, [{ toto: true }])
    ).to.throw("data parameter supplied is not a string.");
  });
  it("should alert if the total satoshis is not enough", () => {
    expect(() =>
      coinSelection(utxosListAsUnspentOutput, [outputs.HUNDRED_DASH])
    ).to.throw(
      "Unsufficient utxos (7099960000) to cover the output : 10000000000. Diff : -2900040000"
    );
  });
  ...
  it("should get a coinSelection for 1 dash", () => {
    const result = coinSelection(
      ...
      false,
      "normal",
      STRATEGIES.simpleDescendingAccumulator
    );
    const expectedResult = {
      utxos: [
        new Transaction.UnspentOutput({
          ...
          outputIndex: 1,
          script: new Script(
            "76a9142f6cb2047c14f0068a561fa2df704e64467ce9c588ac"
          ),
        }),
      ],
      outputs: [
    ...
    const expectedResult = {
      utxos: [
        ...
        new Transaction.UnspentOutput({
          ...
          amount: 1,
          script: new Script(
            "76a9146b1e46d3f3d559dda4468cc30a7b612705eb810f88ac"
          ),
          satoshis: 1 * 1e8,
        }),
        new Transaction.UnspentOutput({
        ...
        new Transaction.UnspentOutput({
          ...
          outputIndex: 1,
          script: new Script(
            "76a9140eb58a39a96968c19411568752ecdecf55dabb8588ac"
          ),
          satoshis: 0.5 * 1e8,
        }),
        new Transaction.UnspentOutput({
          ...
          outputIndex: 0,
          script: new Script(
            "76a914948cf5d360500a04d0a9080eac8514b79c1297b288ac"
          ),
          satoshis: 1.5 * 1e8,
        }),
        new Transaction.UnspentOutput({
          ...
          outputIndex: 1,
          script: new Script(
            "76a91422fef09d745700a159553dd42227895053d33e6888ac"
          ),
          satoshis: 8.4999 * 1e8,
        }),
        new Transaction.UnspentOutput({
        ...
        new Transaction.UnspentOutput({
          ...
          outputIndex: 1,
          script: new Script(
            "76a91419fc1815a04c42a849a7a6dda826c67478514fed88ac"
          ),
          satoshis: 9.9999 * 1e8,
        }),
      ],
    ...
    const expectedResult = {
      utxos: [
        ...
        new Transaction.UnspentOutput({
          ...
          outputIndex: 1,
          script: new Script(
            "76a91425f1c9581cd2a9976e6ace867f8e895663e6825a88ac"
          ),
          satoshis: 6.9998 * 1e8,
        }),
      ],
  ...
  it("should handle externally crafted strategy", function () {
    ...
    const externalStrategy = (
      ...
      while (inputValue < outputValue) {
        ...
        const utxo = copiedUtxos.splice(
          Math.floor(Math.random() * copiedUtxos.length),
          1
        )[0];
